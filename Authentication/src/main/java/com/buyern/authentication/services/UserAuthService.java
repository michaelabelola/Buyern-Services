...
import com.buyern.authentication.repositories.LoginTrackerRepository;
...
public class UserAuthService {
    final LoginTrackerRepository loginTrackerRepository;
...
//        log.info(val.getClaims().toString());
        ...
        loginTracker.setId(userAuth.getId());
        loginTracker.setUserUid(userAuth.getUid());
//        loginTracker.setLastLoginTime();
        loginTrackerRepository.save(loginTracker);

        ObjectNode responseNode = new ObjectMapper().createObjectNode();
        return Map.of("tokens", tokens, "login tracker current status", loginTracker);
//        return tokens;
    }
}
